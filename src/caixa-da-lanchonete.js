class CaixaDaLanchonete {
        ...
        if(resultadoVerificaMetodoDePagamento == ''){
            if(resultadoVerificaItens == ''){
                ...
                if( verificaValidadeDosItens == ''){
                    console.log(this.percorrerCardapio(arrayAux))
    ...
    percorrerCardapio(itens){
        let valor = 0
        for(let i = 0; i < itens.length;i+2){
            let x = this.cardapio.filter(item => item.codigo == itens[i]).map(item => item.valor)
            valor += x
        }
        return valor
    }

    descontosETaxas (metodoDePagamento){
